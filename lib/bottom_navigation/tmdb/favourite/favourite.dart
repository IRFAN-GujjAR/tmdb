...
import 'package:tmdb/bottom_navigation/tmdb/favourite/movies/favourite_movies.dart';
import 'package:tmdb/bottom_navigation/tmdb/favourite/tv_shows/favourite_tv_shows.dart';
...
class _FavouriteState extends State<Favourite>
  ...
  void _initializeFavourite() async {
    setState(() {
      _isLoading = true;
    });

    int length = 0;
    _moviesList = await getFavouriteMovies(
        http.Client(), widget.sessionId, widget.accountId, 1);
    _tvShowsList = await getFavouriteTvShows(
        http.Client(), widget.sessionId, widget.accountId, 1);
            ...
            child: _isLoading
                ...
                : _isInternet
                    ? _resultsNotFound
                        ? Center(
                            child: Text(
                                'No Movie or Tv Show is added to favourite!'),
                          )
                        : Container(
                    ...
                    : InternetConnectionErrorWidget(
                        onPressed: _initializeFavourite,
                      ))
        : Scaffold(
            appBar: AppBar(
