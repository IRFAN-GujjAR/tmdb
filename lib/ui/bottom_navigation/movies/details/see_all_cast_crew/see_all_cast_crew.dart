...
enum _SeeAllCast { Cast, Crew }

class SeeAllCastCrew extends StatefulWidget {
  final String previousPageTitle;
  final models.Credits credits;

  SeeAllCastCrew({@required this.previousPageTitle, @required this.credits});
...
class _SeeAllCastCrewState extends State<SeeAllCastCrew>
  ...
  _SeeAllCast _seeAllCastCrew = _SeeAllCast.Cast;
  ...
  Widget get _buildTabs {
    Map<_SeeAllCast, Widget> tabBars = {};
    if (widget.credits.cast != null && widget.credits.cast.isNotEmpty) {
      tabBars.addAll({_SeeAllCast.Cast: _buildTabTitle('Cast')});
    }
    if (widget.credits.crew != null && widget.credits.crew.isNotEmpty) {
      tabBars.addAll({_SeeAllCast.Crew: _buildTabTitle('Crew')});
    }
    ...
    return Container(
      ...
      child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
        CupertinoSlidingSegmentedControl<_SeeAllCast>(
            groupValue: _seeAllCastCrew,
            children: tabBars,
            onValueChanged: (value) {
              setState(() {
                switch (value) {
                  case _SeeAllCast.Cast:
                    _seeAllCastCrew = _SeeAllCast.Cast;
                    _tabController.index = 0;
                    break;
                  case _SeeAllCast.Crew:
                    _seeAllCastCrew = _SeeAllCast.Crew;
...
class AllCredits extends StatefulWidget {
  ...
  AllCredits({this.cast, this.crew});
